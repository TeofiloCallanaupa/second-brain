...
export function createGithubIssuesTool(userId: string) {
  return withGitHubConnection(
    tool({
      description:
        "Read issues from a GitHub repository. The user must have connected their GitHub account first using the Connect GitHub button in the footer.",
      ...
      execute: async ({ owner, repo, state }) => {
        try {
          ...
          return {
            error:
              "Could not access GitHub. The user needs to connect their GitHub account by clicking the 'GitHub' button in the footer bar and completing the authorization flow, then try again.",
          };
        }
      },
    })
