...
export function createGmailReadTool(userId: string) {
  return withGoogleConnection(
    tool({
      description:
        "Read recent emails from the user's Gmail inbox. Can optionally filter by a search query. The user must have connected their Google account first using the Connect Google button in the footer.",
